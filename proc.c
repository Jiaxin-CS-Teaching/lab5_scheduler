 ...
 * ------------------------
 * change priority of a process [pid]
 *
 *
*/
int set_priority(int pid, int priority)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) // traverse ptable
  {
    ...
    {
      p->priority = priority; // change its priority
      return 0;
    }
  }
...
sched1(void)
{
  ...
  //ptable is a process list
  //it stores all the existing processes in the system
/******** scheduler *********/
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){ // traverse ptable
      if(p->state != RUNNABLE)
        continue;

      //code execution arrives here only when a runnable process is found
 ...
 * to support two different schedulings: default rr and priority-based
...
//  struct proc *p;
  ...
  for(;;){
    ...
    acquire(&ptable.lock);
    // CPS3250
    // switch between default rr and priority-based
